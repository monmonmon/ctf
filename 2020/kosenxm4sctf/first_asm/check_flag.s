...
check_flag:
	...
	mov QWORD PTR [rbp - 0x8], rdi  # 引数文字列のアドレス
	mov QWORD PTR [rbp - 0x10], 0   # カウンタ

	.for_start:
        # 引数の文字列のNバイト目をclに読み込む
		mov rax, QWORD PTR [rbp - 0x8]  # 引数文字列のアドレス
		mov rbx, QWORD PTR [rbp - 0x10] # カウンタの値
